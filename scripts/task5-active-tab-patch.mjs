...
const nextTest = "test('whole-page CLIP classification is gated behind explicit CLIP selection'";
const pattern = /test\('granting full site access enables DOM protection and revocation returns to baseline', async \(\) => \{[\s\S]*?\n\}\);\n\ntest\('whole-page CLIP classification is gated behind explicit CLIP selection'/;
...
if (!pattern.test(source)) throw new Error("Task 5 lifecycle test block not found");
