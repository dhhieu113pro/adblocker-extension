...
class AdBlockerOverlay {
  ...
  handleJwAdSkip() {
    ...
    const readyButton = skipButtons.find((button) => {
      ...
      return button.classList.contains("jw-skippable") &&
        (label === "bỏ qua" || label === "skip ad" || label === "skip" || text === "bỏ qua" || text === "skip ad" || text === "skip");
    });
    ...
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      if (message.type === "getTabDetectedAds") {
        const adsList = Array.from(this.detectedAdsMap.values()).map((ad) => ({
          id: ad.id, url: ad.url, domain: ad.domain, width: ad.width, height: ad.height,
          confidence: ad.confidence, method: ad.method, reasons: ad.reasons, isHidden: ad.isHidden,
        }));
  ...
  scanKnownAdSlots() {
    ...
    slots.forEach((slot) => {
      ...
      if (isAdbroRoot) { this.hideElement(slot); return; }
      ...
      if (image) {
        ...
        if (src && !this.allowedAds.has(src)) {
          this.hideAd(image, { isAd: true, confidence: 99, method: "Known ad slot detector", reasons: ["Known ad banner container"] });
        }
      }
    });
  }
  ...
  shouldAnalyzeImage(img) {
    ...
    // Skip obvious UI/branding assets. Geometry is only used to avoid wasting AI
    // on tiny icons; it is never evidence that an image is an advertisement.
  ...
  isAdIframeCandidate(iframe) {
    ...
    const adIframeKeywords = [
      "vli", "vliifrwrapper", "google_ads", "aswift", "ad-iframe", "ad_iframe", "adframe", "banner", "taboola", "outbrain", "ezoic", "doubleclick",
      "adservice", "adserver", "pubads", "amazon-ads", "criteo", "popads", "mgid", "exoclick", "propeller", "juicyads", "adscatfish", "sspp", "z2-vli",
      "eclick", "smartads", "adsbyeclick", "adnzone", "adxzone", "adx", "admicro", "ssppage", "sspbid", "tvcpzone", "admrick", "mediumiframe", "populartooth",
    ...
    // Keep iframe geometry as a fallback only; this change is specifically for image classification.
    if (width > 0 && height > 0) {
  ...
  scanPlayStreamAdLayers() {
    const adLayers = document.querySelectorAll('[id^="ps-ad-player-player-container"], [id^="ps-ad-player-display-container"], [id^="ps-ad-player-controller"], iframe[title="Advertisement"]');
  ...
  hideAdVideo(video, reason, explicitContainer) {
    ...
    const adInfo = {
      id: adId, url: video.currentSrc || video.src || "Ad Video", domain: "ad video",
      width: video.videoWidth || 0, height: video.videoHeight || 0, confidence: 95,
      method: "Auto video ad detection", reasons: [reason], isHidden: true, targetElement, imgElement: video, originalDisplay,
    };
  ...
  hideAdIframe(iframe) {
    ...
    this.detectedAdsMap.set(adId, {
      id: adId, url: src, domain: domain || "iframe ad", width, height, confidence: 95,
      method: "Iframe Ad Network Detector", reasons: [`Ad Iframe detected (${iframe.id || iframe.className || "ad-frame"})`],
      isHidden: true, targetElement, imgElement: iframe, originalDisplay,
    });
      ...
      const checkAndProcess = () => {
        ...
        for (let i = 0; i < 4 && currEl && currEl.parentElement; i++) {
          ...
          if (closeBtn) {
            ...
            if (btnText.includes("qc") || btnText.includes("quảng cáo") || btnText.includes("advertisement") || ariaLabel.includes("qc") || ariaLabel.includes("quảng cáo") || ariaLabel.includes("advertisement") || ariaLabel.includes("ad close") || className.includes("close-ad") || className.includes("ad-close") || className.includes("no-ads")) {
  ...
  async fetchImageDataUrl(imageUrl) {
    try {
      const fetchRes = await new Promise((resolve) => {
        chrome.runtime.sendMessage({ type: "fetchImageAsBase64", url: imageUrl }, (response) => resolve(response));
      });
      return fetchRes?.base64 || "";
    } catch (err) {
      console.warn("[AdBlocker] Failed to fetch image for AI classification:", err);
      return "";
    }
  }
    ...
    while (this.adCheckQueue.length > 0) {
      ...
      await new Promise((resolve) => {
        chrome.runtime.sendMessage({ type: "detectAd", ...msg, imageDataUrl }, (res) => {
          if (res?.isAd && res.confidence >= 50 && img?.isConnected) {
            this.hideAd(img, res);
            this.cleanupEmptyAdContainers();
          }
          resolve();
  ...
  async analyzeImage(img) {
    try {
      ...
      chrome.runtime.sendMessage({ type: "detectAd", imageUrl: imgSrc, imageDataUrl, width, height, linkUrl, linkRel, hasCloseAdButton, forceAI: true }, (res) => {
        if (res?.success) {
  ...
  showResultModal(img, res) {
    ...
    Object.assign(modal.style, { position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: "2147483647", backgroundColor: "#0f172a", color: "#f8fafc", padding: "24px", borderRadius: "16px", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.7)", fontFamily: "system-ui, -apple-system, sans-serif", maxWidth: "420px", width: "90%", border: "1px solid #334155" });
  ...
  scanClickjackingOverlays() {
    ...
    divs.forEach((el) => {
      ...
      if (!(isBgTransparent || isOpacityZero) || style.pointerEvents === "none") return;
